...
HRESULT myIDirect3DDevice9::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
	...
	//if (SUCCEEDED(res)) PostFX_Initialize(this, pPresentationParameters);
...
HRESULT myIDirect3DDevice9::GetBackBuffer(UINT iSwapChain,UINT iBackBuffer,D3DBACKBUFFER_TYPE Type,IDirect3DSurface9** ppBackBuffer)
{
	...
	if (returnAddress == (void*)0x539BA9)
	{
		rtHandle = NULL;

		IDirect3DSurface9* backBuffer;
		m_pIDirect3DDevice9->GetBackBuffer(iSwapChain, iBackBuffer, Type, &backBuffer);

		D3DSURFACE_DESC desc;
		backBuffer->GetDesc(&desc);

		HRESULT res = m_pIDirect3DDevice9->CreateRenderTarget(desc.Width, desc.Height, desc.Format, D3DMULTISAMPLE_NONE, 0, FALSE, ppBackBuffer, &rtHandle);

		renderTarget = *ppBackBuffer;

		// set it in libgles
		typedef void (__stdcall * SetSharedHandle_t)(HANDLE);

		HMODULE es = LoadLibraryA("libGLESv2.dll");
		SetSharedHandle_t ssh = (SetSharedHandle_t)GetProcAddress(es, "iwSetSharedHandle");
		ssh(rtHandle);
...
{
	/*if (_ReturnAddress() == (void*)0x539BF5)
	{
...
{
	HRESULT hr = m_pIDirect3DDevice9->SetRenderTarget(RenderTargetIndex,pRenderTarget);

	if (SUCCEEDED(hr)) {
		//PostFX_SetRenderTarget(RenderTargetIndex, pRenderTarget);
	}
...
{
	//PostFX_PreBeginScene();
    
	HRESULT res = m_pIDirect3DDevice9->BeginScene();

	m_pIDirect3DDevice9->SetRenderTarget(0, renderTarget);

	if (GetAsyncKeyState(VK_F4))
	{
		OutputDebugString(va("beginscene called from %08x\n", (DWORD)_ReturnAddress()));
	}

	if (SUCCEEDED(res))
	{
		//PostFX_PostBeginScene();
	}
...
{
	m_pIDirect3DDevice9->SetRenderTarget(0, NULL);

    HRESULT res = m_pIDirect3DDevice9->EndScene();
	IDirect3DStateBlock9 * pDeviceState = NULL;

	//this->ShowWeAreHere();

	/*m_pIDirect3DDevice9->BeginScene();
	m_pIDirect3DDevice9->CreateStateBlock ( D3DSBT_ALL, &pDeviceState );
...
static bool isThisSky = false;
static bool isThisBarrel = false;
extern IDirect3DTexture9* barreltexture;

HRESULT myIDirect3DDevice9::SetTexture(DWORD Stage,IDirect3DBaseTexture9* pTexture)
{
	if (barreltexture && pTexture == barreltexture)
	{
		isThisBarrel = true;
	}
...
void DebugVertexShader(void* asset, int ud);
void DebugVertexDecl(void* asset, int ud);
...
void DebugVertexDecl(void* asset, int ud)
{
	IDirect3DVertexDeclaration9* d3decl = (IDirect3DVertexDeclaration9*)ud;
	VertexDecl* decl = (VertexDecl*)asset;

	for (int i = 0; i < 16; i++)
	{
		if (decl->declarations[i] && decl->declarations[i] == d3decl)
		{
			OutputDebugString(va("!!! decl %s[%i]\n", decl->name, i));
		}
	}
}
...
HRESULT myIDirect3DDevice9::SetVertexDeclaration(IDirect3DVertexDeclaration9* pDecl)
{
	if (isThisBarrel)
	{
		DB_EnumXAssets(ASSET_TYPE_VERTEXDECL, DebugVertexDecl, (int)pDecl);
	}
...
void DebugVertexShader(void* asset, int ud)
{
	IDirect3DVertexShader9* d3decl = (IDirect3DVertexShader9*)ud;
	VertexShader* decl = (VertexShader*)asset;

	if (decl->shader == d3decl)
	{
		OutputDebugString(va("!!! shader: %s\n", decl->name));
	}
}
...
HRESULT myIDirect3DDevice9::SetVertexShader(IDirect3DVertexShader9* pShader)
{
	if (isThisBarrel)
	{
		DB_EnumXAssets(ASSET_TYPE_VERTEXSHADER, DebugVertexShader, (int)pShader);
	}
